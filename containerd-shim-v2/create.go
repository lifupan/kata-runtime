...
func doMount(s *service, r *taskAPI.CreateTaskRequest) error {
	...
	if len(r.Rootfs) == 1 {
		...
		if katautils.IsBlockDevice(m.Source) && !runtimeConfig.HypervisorConfig.DisableBlockDeviceUse {
			s.mount = false
		}
	}
